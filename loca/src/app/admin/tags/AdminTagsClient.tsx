    ...
    <div className="w-full md:pl-8">
      ...
      <section className="mt-5 rounded-2xl bg-white p-5 shadow-[0_10px_28px_rgba(24,24,27,0.08)]">
        <div className="flex gap-2">
          <input className="h-12 min-w-0 flex-1 rounded-xl border border-[var(--border)] px-4 text-sm" placeholder="태그명" value={name} onChange={(event) => setName(event.target.value)} />
          <Button onClick={submit}>생성</Button>
        </div>
      </section>
      <section className="mt-5 flex flex-wrap gap-2">
        {tags.map((tag) => (
          <span className="inline-flex items-center gap-2 rounded-full bg-white px-4 py-2 text-sm font-bold shadow-[0_8px_20px_rgba(24,24,27,0.08)]" key={tag.id}>
            {tag.name}
            <button className="text-zinc-400" onClick={() => remove(tag.id)} type="button">삭제</button>
          </span>
        ))}
      </section>
    </div>
