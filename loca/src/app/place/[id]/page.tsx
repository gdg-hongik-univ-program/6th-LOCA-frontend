      ...
      <div className="md:grid md:min-h-[calc(100dvh-56px)] md:grid-cols-[1fr_430px]">
        <section className="relative h-80 overflow-hidden md:h-auto">
          ...
          <div className="absolute inset-x-0 top-4 flex items-center justify-between px-4 text-white md:px-8 md:top-8">
            <Link aria-label={copy.back} className="rounded-full bg-black/20 p-2 backdrop-blur" href="/explore">
              <Icon name="chevron" />
            </Link>
            <div className="flex gap-2">
              <button aria-label={copy.share} className="rounded-full bg-black/20 p-2 backdrop-blur" type="button">
                <Icon name="share" />
              </button>
              <button aria-label={copy.bookmark} className="rounded-full bg-black/20 p-2 backdrop-blur" type="button">
                <Icon name="bookmark" />
              </button>
            </div>
          </div>
        ...
        <section className="-mt-8 rounded-t-[28px] bg-white px-5 pb-28 pt-7 md:mt-0 md:rounded-none md:px-8 md:py-10">
          ...
          <p className="mt-5 text-sm leading-7 text-[var(--text-secondary)] md:text-base">{place.description}</p>
          ...
          <Button className="mt-8 hidden w-full md:inline-flex" href={`/review/write?placeId=${place.id}`}>
            {copy.record}
          </Button>
        </section>
      </div>

      <div className="absolute inset-x-0 bottom-[78px] z-10 bg-white px-5 py-3 md:hidden">
